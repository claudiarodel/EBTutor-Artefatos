...
tags_personalizadas = {
    0: "Duvidas_Clareza_Feedback",
    1: "Elogio_Usabilidade_Relevância_Utilidade",
    2: "Sugestões_Melhorias",
    3: "Problemas_Técnicos"
}
...
df.loc[df["avaliacao"].str.contains("não consegui|infelizmente|erro|bug|travou", case=False, na=False), "cluster_tag"] = "Problemas_Técnicos"

# Outro exemplos...: se contém "sugestão" ou "poderia melhorar", força para "Sugestões"
df.loc[df["avaliacao"].str.contains("Disponibilidade na internet|acesso pela internet|não só intranet|escolher por qualquer assunto|fornt-end|integrada ao EBAula|poderia comentar|poderia mapear|seja feito|pouco intuitivo|outros menus na barra lateral|Apresentar|agrupamento melhor|deve ser melhor|melhor avaliada|deveriam ser|poderia ter|integração|acredito|poderia ser|Orientar|sugestao|poderia melhorar|melhoria|layout|orientar", case=False, na=False), "cluster_tag"] = "Sugestões_Melhorias"

df.loc[df["avaliacao"].str.contains("bom contar com|inovador|muito bem|bom sistema|Ajuda muito|muito boa|útil|bem interessante|excelente|ótimo|auxiliar|esclareceu", case=False, na=False), "cluster_tag"] = "Elogio_Usabilidade_Relevância_Utilidade"


df.loc[df["avaliacao"].str.contains("pouco clara|Não ficou muito claro|não condizente|não entendi", case=False, na=False), "cluster_tag"] = "Duvidas_Clareza_Feedback"

#df.loc[df["avaliacao"].str.contains("nada a acrescentar|mais tempo|período maior|nada a declarar", case=False, na=False), "cluster_tag"] = "Neutro"
...
plt.tight_layout()
# ===== SALVAR =====
plt.savefig("nuvem_palavras_comentarios_adjcmd.png", dpi=300, bbox_inches="tight")
plt.show()




#8. Análise quantitativa: distribuição dos clusters
...
percentual = (distribuicao / 69) * 100
...
#plt.bar(percentual.index, percentual.values, color="skyblue")
...
plt.tight_layout()
# ===== SALVAR =====
plt.savefig("grafico_percentual_clusters_adjcmd.png", dpi=300, bbox_inches="tight")
plt.show()
...
df.to_excel("dados_com_clusters_embeddings_adjcmd.xlsx", index=False)


#--------------------Geração do texto Interpretativo------------------------

# 1. Abrir o arquivo Excel já gerado
df = pd.read_excel("dados_com_clusters_embeddings_adjcmd.xlsx")
...
"""
texto_final = "=== Relatório Final de Interpretação Qualitativa Indutiva ===\n\n"
texto_final += "A análise qualitativa indutiva revelou categorias que emergiram diretamente dos dados. "
...
texto_final += "\n\nEm síntese, os resultados revelam uma boa impressão da ferramenta, acompanhadas por sugestões construtivas para expandir as funcionalidades da ferramenta e dúvidas que apontam para oportunidades de refinamento e melhoria contínua."
texto_final += "Além do mais, relatam as dificuldades técnicas de acesso, encontradas pelos alunos, tendo em vista o EBTutor estar hospedado na intranet do EB."
...
with open("relatorio_final_interpretacao_adjcmd.txt", "w", encoding="utf-8") as f:
    f.write(texto_final)
