...
# 1. Carregar dados
df = pd.read_excel("dados_dqbrn_sgt.xlsx")
...
df.loc[df["avaliacao"].str.contains("deveria ser|aprimorado|acesso na internet|disponibilizar na internet|Poderia|poderia|Apresentar|agrupamento melhor|deve ser melhor|melhor avaliada|deverdqbrn_sgt ser|poderia ter|integração|acredito|poderia ser|Orientar|sugestao|poderia melhorar|melhoria|layout|orientar", case=False, na=False), "cluster_tag"] = "Sugestões_Melhorias"

df.loc[df["avaliacao"].str.contains("muito interessante|bons|Muito bom|CHA|Muito boa|muito úteis|excelente|ótimo|auxiliar|esclareceu|excelente", case=False, na=False), "cluster_tag"] = "Elogio_Usabilidade_Relevância_Utilidade"
...
plt.tight_layout()
# ===== SALVAR =====
plt.savefig("nuvem_palavras_comentarios_dqbrn_sgt.png", dpi=300, bbox_inches="tight")
plt.show()
...
plt.tight_layout()
# ===== SALVAR =====
plt.savefig("grafico_percentual_clusters_dqbrn_sgt.png", dpi=300, bbox_inches="tight")
plt.show()
plt.show()
...
df.to_excel("dados_com_clusters_embeddings_dqbrn_sgt.xlsx", index=False)


#--------------------Geração do texto Interpretativo------------------------

# 1. Abrir o arquivo Excel já gerado
df = pd.read_excel("dados_com_clusters_embeddings_dqbrn_sgt.xlsx")
...
with open("relatorio_final_interpretacao_dqbrn_sgt.txt", "w", encoding="utf-8") as f:
    f.write(texto_final)
