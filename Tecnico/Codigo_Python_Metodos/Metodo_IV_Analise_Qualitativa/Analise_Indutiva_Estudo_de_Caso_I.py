...
# 1. Carregar dados
df = pd.read_excel("dados_redes_sgt.xlsx")
...
df.loc[df["avaliacao"].str.contains("Apresentar|agrupamento melhor|deve ser melhor|melhor avaliada|deveriam ser|poderia ter|integração|acredito|poderia ser|Orientar|sugestao|poderia melhorar|melhoria|layout|orientar", case=False, na=False), "cluster_tag"] = "Sugestões_Melhorias"

df.loc[df["avaliacao"].str.contains("excelente|ótimo|auxiliar|esclareceu", case=False, na=False), "cluster_tag"] = "Elogio_Usabilidade_Relevância_Utilidade"
...
plt.tight_layout()
# ===== SALVAR =====
plt.savefig("nuvem_palavras_comentarios_redessgt.png", dpi=300, bbox_inches="tight")
plt.show()
...
# Calcular percentual
percentual = (distribuicao / 13) * 100
...
plt.tight_layout()
# ===== SALVAR =====
plt.savefig("grafico_percentual_clusters_redessgt.png", dpi=300, bbox_inches="tight")
plt.show()
plt.show()
...
df.to_excel("dados_com_clusters_embeddings_redes_sgt.xlsx", index=False)


#--------------------Geração do texto Interpretativo------------------------

# 1. Abrir o arquivo Excel já gerado
df = pd.read_excel("dados_com_clusters_embeddings_redes_sgt.xlsx")
...
with open("relatorio_final_interpretacao_redes_sgt.txt", "w", encoding="utf-8") as f:
    f.write(texto_final)
