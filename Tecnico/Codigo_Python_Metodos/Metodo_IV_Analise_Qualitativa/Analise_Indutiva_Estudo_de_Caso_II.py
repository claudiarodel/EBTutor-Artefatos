...
# 1. Carregar dados
df = pd.read_excel("dados_redes_of.xlsx")
...
df.loc[df["avaliacao"].str.contains("vpn|infelizmente|erro|bug|travou", case=False, na=False), "cluster_tag"] = "Problemas_Técnicos"

# Outro exemplos...: se contém "sugestão" ou "poderia melhorar", força para "Sugestões"
df.loc[df["avaliacao"].str.contains("fornt-end|integrada ao EBAula|poderia comentar|poderia mapear|seja feito|pouco intuitivo|outros menus na barra lateral|Apresentar|agrupamento melhor|deve ser melhor|melhor avaliada|deveriam ser|poderia ter|integração|acredito|poderia ser|Orientar|sugestao|poderia melhorar|melhoria|layout|orientar", case=False, na=False), "cluster_tag"] = "Sugestões_Melhorias"

df.loc[df["avaliacao"].str.contains("Muito bom|Ajuda muito|muito boa|útil|bem interessante|excelente|ótimo|auxiliar|esclareceu", case=False, na=False), "cluster_tag"] = "Elogio_Usabilidade_Relevância_Utilidade"


df.loc[df["avaliacao"].str.contains("Não ficou muito claro|não condizente|não entendi", case=False, na=False), "cluster_tag"] = "Duvidas_Clareza_Feedback"
...
plt.tight_layout()
# ===== SALVAR =====
plt.savefig("nuvem_palavras_comentarios_redesof.png", dpi=300, bbox_inches="tight")
plt.show()
...
# Calcular percentual
percentual = (distribuicao / 30) * 100
...
plt.tight_layout()
# ===== SALVAR =====
plt.savefig("grafico_percentual_clusters_redesof.png", dpi=300, bbox_inches="tight")
plt.show()
plt.show()
...
df.to_excel("dados_com_clusters_embeddings_redes_of.xlsx", index=False)


#--------------------Geração do texto Interpretativo------------------------

# 1. Abrir o arquivo Excel já gerado
df = pd.read_excel("dados_com_clusters_embeddings_redes_of.xlsx")
...
texto_final += "\n\nEm síntese, os resultados revelam uma boa impressão da ferramenta, acompanhadas por várias sugestões construtivas para expandir as funcionalidades da ferramenta e algumas dúvidas que apontam para oportunidades de refinamento e melhoria contínua. ."
...
with open("relatorio_final_interpretacao_redes_of.txt", "w", encoding="utf-8") as f:
    f.write(texto_final)
