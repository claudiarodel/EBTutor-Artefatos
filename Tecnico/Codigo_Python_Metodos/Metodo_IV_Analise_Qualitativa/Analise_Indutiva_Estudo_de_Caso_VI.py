...
# 1. Carregar dados
df = pd.read_excel("dados_casipex.xlsx")
...
df.loc[df["avaliacao"].str.contains("Não necessitar|Interação|Designe|aprimorado|acesso na internet|disponibilizar na internet|Poderia|poderia|Apresentar|agrupamento melhor|deve ser melhor|melhor avaliada|devercasipex ser|poderia ter|integração|acredito|poderia ser|Orientar|sugestao|poderia melhorar|melhoria|layout|orientar", case=False, na=False), "cluster_tag"] = "Sugestões_Melhorias"
...
plt.tight_layout()
# ===== SALVAR =====
plt.savefig("nuvem_palavras_comentarios_casipex.png", dpi=300, bbox_inches="tight")
plt.show()
...
# Calcular percentual
percentual = (distribuicao / 20) * 100
...
plt.tight_layout()
# ===== SALVAR =====
plt.savefig("grafico_percentual_clusters_casipex.png", dpi=300, bbox_inches="tight")
plt.show()
...
df.to_excel("dados_com_clusters_embeddings_casipex.xlsx", index=False)


#--------------------Geração do texto Interpretativo------------------------

# 1. Abrir o arquivo Excel já gerado
df = pd.read_excel("dados_com_clusters_embeddings_casipex.xlsx")
...
with open("relatorio_final_interpretacao_casipex.txt", "w", encoding="utf-8") as f:
    f.write(texto_final)
