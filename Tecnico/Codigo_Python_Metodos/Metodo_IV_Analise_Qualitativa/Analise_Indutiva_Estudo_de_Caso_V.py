...
# 1. Carregar dados
df = pd.read_excel("dados_dqbrn_of.xlsx")
...
df.loc[df["avaliacao"].str.contains("acesso na internet|disponibilizar na internet|Poderia|poderia|Apresentar|agrupamento melhor|deve ser melhor|melhor avaliada|deverdqbrn_of ser|poderia ter|integração|acredito|poderia ser|Orientar|sugestao|poderia melhorar|melhoria|layout|orientar", case=False, na=False), "cluster_tag"] = "Sugestões_Melhoria"

df.loc[df["avaliacao"].str.contains("Muito boa|muito úteis|excelente|ótimo|auxiliar|esclareceu|excelente", case=False, na=False), "cluster_tag"] = "Elogio_Usabilidade_Relevância_Utilidade"
...
plt.tight_layout()
# ===== SALVAR =====
plt.savefig("nuvem_palavras_comentarios_dqbrn_of.png", dpi=300, bbox_inches="tight")
plt.show()
...
plt.tight_layout()
# ===== SALVAR =====
plt.savefig("grafico_percentual_clusters_dqbrn_of.png", dpi=300, bbox_inches="tight")
plt.show()
plt.show()
...
df.to_excel("dados_com_clusters_embeddings_dqbrn_of.xlsx", index=False)


#--------------------Geração do texto Interpretativo------------------------

# 1. Abrir o arquivo Excel já gerado
df = pd.read_excel("dados_com_clusters_embeddings_dqbrn_of.xlsx")
...
with open("relatorio_final_interpretacao_dqbrn_of.txt", "w", encoding="utf-8") as f:
    f.write(texto_final)
