...
# 1. Carregar dados
df = pd.read_excel("dados_iam.xlsx")
...
df.loc[df["avaliacao"].str.contains("não constou|não consegui utilizar|mozilla|não apareceu|Não abre|não funciona|não consegui|vpn|infelizmente|erro|bug|travou|problemas técnicos|não funcionou", case=False, na=False), "cluster_tag"] = "Problemas_Técnicos"

# Outro exemplos...: se contém "sugestão" ou "poderia melhorar", força para "Sugestões"
df.loc[df["avaliacao"].str.contains("acesso na internet|disponibilizar na internet|Poderia|poderia|Apresentar|agrupamento melhor|deve ser melhor|melhor avaliada|deveriam ser|poderia ter|integração|acredito|poderia ser|Orientar|sugestao|poderia melhorar|melhoria|layout|orientar", case=False, na=False), "cluster_tag"] = "Sugestões_Melhorias"

df.loc[df["avaliacao"].str.contains("excelente|ótimo|auxiliar|esclareceu|excelente", case=False, na=False), "cluster_tag"] = "Elogio_Usabilidade_Relevância_Utilidade"


df.loc[df["avaliacao"].str.contains("não condizente|não entendi", case=False, na=False), "cluster_tag"] = "Duvidas_Clareza_Feedback"
...
plt.tight_layout()
# ===== SALVAR =====
plt.savefig("nuvem_palavras_comentarios_iam.png", dpi=300, bbox_inches="tight")
plt.show()
...
# Calcular percentual
percentual = (distribuicao / 18) * 100
...
plt.tight_layout()
# ===== SALVAR =====
plt.savefig("grafico_percentual_clusters_iam.png", dpi=300, bbox_inches="tight")
plt.show()
plt.show()
...
df.to_excel("dados_com_clusters_embeddings_iam.xlsx", index=False)


#--------------------Geração do texto Interpretativo------------------------

# 1. Abrir o arquivo Excel já gerado
df = pd.read_excel("dados_com_clusters_embeddings_iam.xlsx")
...
with open("relatorio_final_interpretacao_iam.txt", "w", encoding="utf-8") as f:
    f.write(texto_final)
