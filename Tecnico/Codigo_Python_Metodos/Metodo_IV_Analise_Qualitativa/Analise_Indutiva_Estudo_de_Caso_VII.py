...
# 1. Carregar dados
df = pd.read_excel("dados_idt_bio.xlsx")
...
# 6. Substituir tags automáticas por rótulos personalizados
tags_personalizadas = {
    0: "Duvida_Clareza_Feedback",
    1: "Elogio_Usabilidade_Relevância_Utilidade",
    2: "Sugestões_Melhorias",
    3: "Problemas_Técnicos",
    4: "Compreensão_Sistema"
}
...
df.loc[df["avaliacao"].str.contains("infelizmente|erro|bug|travou", case=False, na=False), "cluster_tag"] = "Problemas_Técnicos"

# Outro exemplos...: se contém "sugestão" ou "poderia melhorar", força para "Sugestões"
df.loc[df["avaliacao"].str.contains("mais rápida|sugestão|acredito|poderia ser|Orientar|sugestao|poderia melhorar|melhoria|layout|orientar", case=False, na=False), "cluster_tag"] = "Sugestões_Melhorias"

df.loc[df["avaliacao"].str.contains("ótimo|auxiliar|esclareceu", case=False, na=False), "cluster_tag"] = "Elogio_Usabilidade_Relevância_Utilidade"


df.loc[df["avaliacao"].str.contains("pouco clara|não condizente|não entendi", case=False, na=False), "cluster_tag"] = "Duvidas_Clareza_Feedback"

df.loc[df["avaliacao"].str.contains("como utilizar", case=False, na=False), "cluster_tag"] = "Compreensão_Sistema"
...
plt.tight_layout()
# ===== SALVAR =====
plt.savefig("nuvem_palavras_comentarios_idtbio.png", dpi=300, bbox_inches="tight")
plt.show()
...
# Calcular percentual
percentual = (distribuicao / 21) * 100
...
plt.tight_layout()
# ===== SALVAR =====
plt.savefig("grafico_percentual_clusters_idtbio.png", dpi=300, bbox_inches="tight")
plt.show()
plt.show()
...
df.to_excel("dados_com_clusters_embeddings_idt_bio.xlsx", index=False)


#--------------------Geração do texto Interpretativo------------------------

# 1. Abrir o arquivo Excel já gerado
df = pd.read_excel("dados_com_clusters_embeddings_idt_bio.xlsx")
...
texto_final += "\n\nEm síntese, os resultados revelam uma boa impressão da ferramenta, assim como as dificuldades técnicas de acesso, encontradas pelos alunos, tendo em vista o EBTutor estar hospedado na intranet do EB."
texto_final += "Além de sugestões construtivas para expandir as funcionalidades da ferramenta e dúvidas, que apontam para oportunidades de refinamento e melhoria contínua."
...
with open("relatorio_final_interpretacao_idt_bio.txt", "w", encoding="utf-8") as f:
    f.write(texto_final)
